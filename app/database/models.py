...
ChartTypeEnum = Enum('STD', 'DX', name='chart_type_enum', validate_strings=True)
ChartDifficultyEnum = Enum(
	'ReMASTER',
	'MASTER',
	'EXPERT',
	'ADVANCED',
...
class Chart(TimestampMixin, Base):
    ...
    chart_type: Mapped[str] = mapped_column(ChartTypeEnum, nullable=False)
    difficulty: Mapped[str] = mapped_column(ChartDifficultyEnum, nullable=False)
    ...
    def __repr__(self) -> str:  # pragma: no cover - repr utility
    	return (
    		f"<Chart id={self.id!r} song_id={self.song_id!r} "
...
class Record(TimestampMixin, Base):
    ...
    rank: Mapped[str] = mapped_column(RecordRankEnum, nullable=False)
    sync: Mapped[str] = mapped_column(RecordSyncEnum, nullable=False, default='')
    combo: Mapped[str] = mapped_column(RecordComboEnum, nullable=False, default='')
